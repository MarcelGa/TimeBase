...
try:
    from .generated import provider_pb2 as provider_pb2  # type: ignore[reportMissingImports]
    from .generated import provider_pb2_grpc as provider_pb2_grpc  # type: ignore[reportMissingImports]
except ImportError:
    # Fallback for when protobuf files haven't been generated yet
    provider_pb2 = cast(Any, None)
    provider_pb2_grpc = cast(Any, None)


class DataProviderServicer(provider_pb2_grpc.DataProviderServicer):
    ...
    async def GetHistoricalData(self, request, context):
        ...
        try:
            ...
            async for data_point in self.provider.get_historical_data(  # type: ignore
    ...
    async def StreamRealTimeData(self, request_iterator, context):
        ...
        try:
            ...
            async for data_point in self.provider.stream_realtime_data(subscription_stream()):  # type: ignore
    ...
    async def GetSymbols(self, request, context):
        """Return provider-supported symbols."""
        try:
            symbols = await self.provider.get_symbols()

            symbol_list = provider_pb2.SymbolList()
            for symbol in symbols:
                entry = provider_pb2.SymbolInfo(
                ...
                symbol_list.symbols.append(entry)
            ...
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(str(e))
            return provider_pb2.SymbolList()
